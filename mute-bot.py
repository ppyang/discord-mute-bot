...
async def invite(ctx):
    await ctx.send("Invite Link: <https://discord.com/api/oauth2/authorize?client_id=777&permissions=8&scope=bot>")
...
async def mute(ctx):
    ...
    try:
        ...
        await ctx.channel.send(f"Something went wrong. Try rejoining the VC. Also make sure the bot has the following "
                               ...
                               f"`Mute Members`. Please contact `wanpeng` if this keeps happening.")
        ...
        me = client.get_user(replace_with_userid)
    ...
    try:
        if ctx.author.voice:  # check if the user is in a voice channel
            if ctx.author.guild_permissions.mute_members:  # check if the user has mute members permission
                ...
                await ctx.channel.send("You don't have the `UnMute Members` permission")
        else:
            await ctx.send("You must join a voice channel first")
